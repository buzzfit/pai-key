...
/**
 * Stable version – prefills wallet from /api/me cookie.
 * Button now shows solid matrix-green background **and black text** so
 * “Dock Agent” is readable.
 */
...
export default function VendorDockForm({ onSubmit, onClose, onConnectWallet }) {
  ...
  /* Prefill wallet if the cookie exists */
  useEffect(() => {
    (async () => {
      try {
  ...
  const allFilled = Object.entries(state).every(([k, v]) =>
    k === 'proof' || k === 'xrpAddr' ? true : Boolean(v)
  );
  ...
  const handleSubmit = e => {
    e.preventDefault();
    if (!canSubmit) return;
    const { agentType, name, tagline, description, capabilities,
            hourlyRate, minHours, proof, xrpAddr } = state;
    onSubmit({
      agentType,
      name,
      tagline,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <form onSubmit={handleSubmit}
            className="relative w-full max-w-xl max-h-[90vh] overflow-y-auto
                       rounded-lg bg-white p-6 space-y-4 dark:bg-gray-800">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block">
            ...
            <input required type="number" min="0.000001" step="0.000001"
