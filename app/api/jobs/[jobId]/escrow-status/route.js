...
export async function GET(request, { params }) {
  ...
  const result = await jobsService.confirmEscrowDeposit({ jobId: params.jobId, hirerWallet: auth.account });
  if (result.error) return jsonError(result.status, result.error[0], result.error[1]);
  return NextResponse.json({ ok: true, job: result.job, escrowStatus: result.payload });
}
