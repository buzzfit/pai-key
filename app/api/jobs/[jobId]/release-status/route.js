...
export async function GET(request, { params }) {
  ...
  const result = await jobsService.confirmEscrowRelease({ jobId: params.jobId, hirerWallet: auth.account });
  if (result.error) return jsonError(result.status, result.error[0], result.error[1]);
  return NextResponse.json({ ok: true, job: result.job, releaseStatus: result.payload });
}
