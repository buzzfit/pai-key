...
export default function VendorDockPage() {
  ...
  /* 2 — helper to refresh agent list */
  const loadAgents = async (acct) => {
    const data = await fetch(`/api/agents?account=${acct}`, { cache: 'no-store' })
                  .then(r => r.json());
    setAgents(data.agents || []);
    return (data.agents || []).length;          // handy count for callers
  };

  useEffect(() => { if (account) loadAgents(account); }, [account]);

  /* 3 — delete handler with auto-logout */
  const deleteAgent = async (id) => {
    /* remove on the server */
    await fetch(`/api/agents/${id}`, { method: 'DELETE' });

    /* refresh local list and get how many remain */
    const remaining = await loadAgents(account);

    /* if none left → clear cookies & return to signup */
    if (remaining === 0) {
