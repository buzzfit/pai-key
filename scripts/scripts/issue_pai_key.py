#!/usr/bin/env python3
"""
Quick demo: create a PAI Key on XRPL testnet.
Usage:
...
def main():
    ...
    ap.add_argument("--deadline", default="48h")
    args = ap.parse_args()

    user_wallet = Wallet(seed=args.seed, sequence=0)
    info = client.request(AccountInfo(account=user_wallet.classic_address, ledger_index="current")).result
    seq = info["account_data"]["Sequence"]

    # 1. add agent as signer
    sls = SignerListSet(
    ...
    # 2. create escrow (testnet drops)
    esc = EscrowCreate(
        account=user_wallet.classic_address,
        amount=str(args.limit * 1000000),  # to drops
